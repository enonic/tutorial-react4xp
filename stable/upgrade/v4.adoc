...
[IMPORTANT]
====
By default React4xp apps serve a globals bundle to the browser.
This globals bundle contain react (and more).
If multiple copies of react are loaded in the browser, you will probably run into problems:
...
WARNING: If you don't pass on the request to the render functions React4xp will assume `edit` mode.
