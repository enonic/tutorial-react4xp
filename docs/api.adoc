...
This chapter also covers the <<#ssr, SSR engine>>, compilation, and <<config/build-time#, configuration>> of those.
...
- Don't build React4xp components with `NODE_ENV = development`, but use `production` (which is the default - see <<config/build-time#NODE_ENV,Build environment>>). Assets built with `development` are much more verbose, and this size difference - although functionally equal - actually makes a difference.
