...
+
. Add your brand new "React4XP" application to the site.
+
image:add-to-site.png[title="Add React4XP application to the site", width=647px]
+
After saving, the preview will refresh, to reveal something like this:
+
image:hello.png[title="React4XP starter page", width=547px]

The component was initially rendered on the server, then <<appendix/hydfation#, hydrated>> to spin up as a client-side React component. 

TIP: Try clicking the button for some React interactivity.
...
The processor does not execute by itself, it must be registered in the dataFetcher. 
...
TIP: Dive deeper by learning how the basic boot-strapping and routing works in the <<appendix/controller#, app controller section>>.
