...
First, open _react.properties_ in your project root and look up the attributes `entryDirs` and `chunkDirs`. These values are comma-separated lists, so make sure `entries` are added as an `entryDir` and `shared` is added as a `chunkDir`:
...
chunkDirs = shared    <!--2-->
----
<1> Entrydirs are relative to _src/main/resource/react4xp/_, and more than one relative path can be added here, separated by commas. Think of this as a list of where, in addition to XP's _site/_ folder, react4xp will look for .JSX source files to 
