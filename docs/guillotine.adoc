...
Time to add some code to the existing _MovieList.jsx_ so it can fetch data from the guillotine endpoint. To easily get started with that, we'll use the other <<#guillotine_helpers, helper module>> included in the react4xp starter: _headless/guillotineRequest.es6_. This too has some convenience error handling and boilerplate like default parameter values/functions, but if we skip that, the bare essence is a `fetch` wrapper:
...
But `setState` is an asynchronous function, and calling it with a "current state of affairs" as the argument runs the risk of introducing race conditions: we'd lose control of timing when the DOM updates, especially since we're going to combine that length-of-DOM with a scrolling trigger. A callback function argument works around this: _"update the state based on what it is at the time this function is called"_.
====
...
image:click-fill-dom.png[title="The request adds new movie data to the state", width=1024px]
