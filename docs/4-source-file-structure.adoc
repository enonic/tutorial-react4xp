...
For react components to be available to React4xp the way we've seen so far, their *JSX source files must be located in certain folders* where react4xp is looking for them and auto-compiles them (they must also export a function of <<entries#_how_to_make_an_entry, props->component>>, but we'll focus on the file locations for now).

React components like that are called *entries* in react4xp.
