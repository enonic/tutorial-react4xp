...
[[ssr]]
== The serverside-rendering engine
...
[[react4xp_properties]]
== Config: react4xp.properties

Some aspects of react4xp can be adjusted with a file _react4xp.properties_. Several of these settings are relevant to the SSR engine.

If you're using the <<setup#create_project, starter>> or your project is based on it, this is already set up and you can <<#buildEnv, skip>> the following section:
...
# Corresponds to --persistent-code-cache, --class-cache-size=1000:
ssrSettings = 1000
...
Persistent code cache is switched off by choice, since it occasionally proved error-prone and didn't improve performance all that much. Your mileage may vary, use with care.
