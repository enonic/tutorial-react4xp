...
<2> Again, since we didn't check the "Server-side rendering" checkbox, the `clientRender` flag became true and we got a clientside rendering. So this is just the target container element, filled in by the browser on `render`. Had we checked the checkbox, this would have contained the pre-rendered MultiColor entry, of course.
