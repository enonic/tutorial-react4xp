...
1. However, we won't use the `React4xp.render` shorthand function from before. Instead, we'll look at a more *explicit react4xp syntax*:
  ...
  - Here, each entry is rendered in *two separate methods*, for `body` and `pageContributions`.
...
3. In addition to chain-rendering more than one entry, we will use the link:#jsxPath[_jsxPath_] as described above to point to entries in a _different location_. Actually, let's just recycle the entries from before:
...
<1> We're using the existing entry link:#hello_react_jsx_modified[_hello-react.jsx_] (with props, not the first hardcoded one). This is located under _site/*pages*/hello-react/_ - even though what we're making here is a _custom-flow_ *Part*. Location doesn't matter: when using jsxPaths, all entries are equal.
<2> This is the existing entry link:#color_jsx[_color.jsx_].
