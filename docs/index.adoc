...
Here we'll modify the files you just made, and look at how to skip the server-side rendering and just render the same react component in the old-school, purely client-side way.

We'll also add some props so that editorial data from XP is displayed in react, routing it in through the props.

React4xp is made to cover many ways of combining XP and react. Let's start with a simple scenario: just displaying a working react component.
