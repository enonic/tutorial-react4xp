...
Now you're ready to get started with the *5 example lessons* - starting from the basics:

- link:#hello_react_page[1: Hello React Page]
- link:#props_clientside[2: Editorial props, client-side]
- link:#xp_pages_parts[3: XP pages and parts]
- link:#custom_flow[4: Custom flow]
- link:#chaining_imports_chunks[5: Chaining]
...
If you clicked the rendering *inside XP Content Studio* instead of a preview tab, you'd notice there was *no bottle-counting*. You see the rendering, but instead of being active react, it's just a regular static XP preview. This way, you get a placeholder/visualization of the react rendering, but can work with it in the XP workflow (react functionality may intervene with both the workflow or even Content Studio itself). This is why we added the `request` argument in `React4xp.render`.

NOTE: Later, we'll look at two other rendering functions: `.renderBody` and `.renderPageContributions`. These are intended for use cases where it's good to be more explicit than `React4xp.render`. For that reason, they _don't automatically handle this for you_. Usually, you should still keep your components from being client-activated inside Content Studio, but you'll have to handle it yourself. link:#custom_flow[Example 3 shows you how].

==== First-serverside-rendering lag
...
Curious about what happened in the rendering? Open the page source code in the browser to look at the preview page. Here's what `React4xp.render` created:
...
[[props_clientside]]
== 2: Editorial props, client-side

Here we'll look at how to skip the server-side rendering and just render the same react component in the purely old-school clientside way.

We'll also add some props with editorial data from XP, displayed in the react component.

We'll do this by modifying the same files you created above:
...
First off, we'll make the component render on the client instead of on the server. That's a one-line operation:
...
----
<1> This (line 23) is all that's needed: a truthy `clientRender` flag in the _options_ object makes the server skip the HTML rendering (in _this_ particular call. Mixing up clientside and serverside rendering across different places is possible. Just not tested very much). And on the client, `React4xp.CLIENT.render` (which wraps `react.render) is called instead of `React4xp.CLIENT.hydrate` - making the browser fill the target element DOM from scratch.

NOTE: As link:#static_cs_rendering[before], this only applies _outside_ of Content Studio. The `clientRender` flag does not change anything inside Content Studio: still server-side rendered static HTML.

==== Props

Now let's change the files some more so we can use Content Studio to input some data into the react rendering.

We'll start by modifying the *react component* so that it displays data from the `props` object, instead of hardcoding everything.
...
function makeObjectDropper(droppableProp, initialCountProp) {
...
export default (props) => <div onClick={makeObjectDropper(props.droppable, props.initialCount)}>
    <h1>{props.message} {props.messageTarget}!</h1>
    <p>Click me: <span id="counter">{props.initialCount}</span> {props.droppable} on the wall.</p>
</div>;
----

[TIP]
====
`props` are of course the standard react way to do this. It _all_ is. As long as a _props => reactComponent_ function is default-exported, react4xp accepts standard valid ES6/JSX.

Although, if you think that _makeObjectDropper_ closure thing below is a strange way to do something react itself could do better, I won't argue. Just trying to focus on one thing at a time.
====

Next, we're modifying *the controller* to inject data into the react component:
...
<1> The `props` are just any standard JS object, directly passed into the react component. One caveat: `props` needs to be serializable - functions can't be passed here!
<2> This makes `React4xp.render` pretty analogous to XP's `thymeleaf.render` template engine. The first argument is just a reference to what should be rendered (_react component_ ~ _template_), the second one (_props_ ~ _model_) is a collection of data injected into it.
<3> `clientRender` has nothing to do with props, of course.

Okay, we're done with the `props` injection, but the data are still hardcoded - only in the controller instead of the react component. The last step is to get the data from XP.

==== Editorial data

This part is just standard XP procedure.

We define the data in the *page definition* XML file. This file is the same as in example 1,but we're adding a few link:https://developer.enonic.com/docs/xp/stable/cms/input-types[input fields] under `<form>`. `greeting`, `greetee`, `counted` and `startCount` are the basis for the initial props of the react component.
...
And we return to the *page controller* to hook the dynamic XP content into the props:
...
exports.get = function(request) {
    ...
    const props = {
        message: pageConfig.greeting,                       // <2>
        messageTarget: pageConfig.greetee,
        droppable: pageConfig.things,
...
<1> Fetching the content data (and in the next line: in this case, unpacking it while ensuring that missing data should just display emptiness, not throw an error)
<2> Connecting the field names in `pageConfig` from _hello-react.xml_ to the prop names that _hello-react.jsx_ needs.
...
You should still see in the preview panel on the right:

[.thumb]
image:hello_bottles.png[title="Rendering: Hello World, same as before. You clicked this image too, didn't you?", width=768px]

But now, when you click "Hello world" (or anywhere on the preview panel), the page-config editing panel opens on the right:

[.thumb]
image:hello_editorial.png[link="media/hello_editorial.png",title="Editing Hello World in the page-config panel in Content Studio. You actually CAN click this image to see a high-res version!", width=1024px]

Of course, this is too similar to the previous example, just repeating "Hello World" might cause a little confusion. Try editing the data in the fields, for example changing _"world"_ into _"link:https://montypython.fandom.com/wiki/Bruces_sketch[Bruce]"_, etc, to keep it clear.

*Apply/save* your new props, and the output should change: since we're still in Content Studio you get a static serverside-rendered update immediately.

Open the page in a *Preview* tab to see the final rendering with your new data, all active:

[.thumb]
image:gday_bruce.png[title="Rendering: G'day Bruce. This amounted to one big philosopher's joke", width=768px]


=== Output

So did anything change in the rendered response, compared to the first example? Not all that much, actually. Depending on what data you inserted and the resulting `props`, your page source should look something like this:
...
----
<1> The target container `react4xpApp` is now empty on the first response from the server. Using `clientRender: true` makes the server trusts the browser to use React to fill in the content.
<2> There's no change in the asset URLs. But since we changed _hello-react.jsx_, the _content_ of _hello-react.js_ has of course been recompiled.
<3> The client wrapper calls `.render` instead of `.hydrate` since we flagged the rendering with `clientRender`. This causes client-side react to fill in the target container with a DOM rendering from scratch...
<4> ...based on the `props` we injected.

[NOTE]
====
If we hadn't used `clientRender`, this output would be identical, except:

- The target container would arrive pre-filled with an HTML representation like the one in example 1, but with your texts from `props` already there.
...
Now might be a good time to take a closer look at the `.render` call from the ES6 controller in link:#_code[the example above]:
...
[[chaining_imports_chunks]]
== 5: Chaining, imports, chunks
