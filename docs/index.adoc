...
- link:#hello_react_page[1: Hello React. Server/clientside]
...
[[hello_react_page]]
== 1: Hello React. Server/clientside
...
Curious about what happened in the rendering of the preview page? Open the *page source* code in the browser. Here's what `React4xp.render` created - the response the client receives on the initial page request:
...
Worth mentioning: a special case where you might want to _temporarily_ clientside-render a component:
...
Often, switching over to _clientside rendering_ for that particular react component, will give you a better/sourcemapped error message in the browser console, making your debugging a bit easier.
====
...
Here we'll spice up our react component a little by using `props` to insert editorial data from XP into the react component.
