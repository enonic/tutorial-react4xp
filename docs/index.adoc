...
The *HTML output* of all this? It's structurally and conceptually the same as in the previous examples. Looking at the _Page Source_, it should all make sense to you by now. One thing is worth noting though: using a react4xp Part more than once does _not_ make the client download the assets more than once - they're recycled for performance.
