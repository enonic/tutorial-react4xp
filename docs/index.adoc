...
[[imports_chunks]]
== 6: Imports, dependency chunks, custom webpack

This final example is going to be more reallife-like. We'll focus on these features:

- Using an entry from a *shared entries* folder.
- The entry itself imports and nests react components. Imports like this are just *regular frontend ES6, and happen at compile time*, done by webpack.
- We'll turn these secondary imports into a separate *dependency chunk*, keeping the footprint of the entry small and leaving the bigger stuff to a repeatedly-used and effectively-cached chunk with shared secondary components.
- Our react components will need some compilation that react4xp doesn't handle. So we'll see how to *add custom webpack config* to the react4xp setup.
