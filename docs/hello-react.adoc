...
[NOTE]
====
A quick note on using `document` and `window` etc, during serverside vs clientside rendering: `document` only works in the browser (although react4xp has polyfilled it as an _empty object_ in nashorn), so code like this should be prevented from running during serverside rendering. In this case, `dropBottle` is the response to a click listener which won't happen on the server, so we don't need any explicit safeguard.

But if you do, it's easy enough to **keep code running only in the browser**. For example:
[source,javascript,options="nowrap"]
----
