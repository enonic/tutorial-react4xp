...
<3> This one is a little bit different compared to previously. `"MultiColor"` is a link:#jsxPath[jsxPath] to an entry, but why does it look so... naked? Where does it point to? As we'll see link:#[later PUUUUUT AAAA LIIIIINK HEEEERE], we'll set up react4xp to look for entries below an `entryDir` in react4xp.properties: _src/main/resources/react4xp/entries/_. Since a jsxPath is relative to the closest parent `entryDir` and we'll put an entry _MultiColor.jsx_ at the root of _entries/_, the jsxPath becomes just the clean file name.
...
Next, we'll add some *custom webpack rules to handle the style imports*. First, we need to tell react4xp to use an extra webpack config file, with another reference in _react4xp.properties_:
...
Even though it's composed of multiple components, this is still a *single XP Part*.

You can add more than one into the Region, and each Part will host an independent react root app.

And let's take a look at the rendered response from the controller:
