...
<3> This one is a little bit different compared to previously. `"MultiColor"` is a link:#jsxPath[jsxPath] to an entry, but why does it look so... naked? Where does it point to? As we'll see link:#_[later PUUUUUT AAAA LIIIIINK HEEEERE], we'll set up react4xp to look for entries below an `entryDir` in react4xp.properties: _src/main/resources/react4xp/entries_. Since a jsxPath is relative to the closest parent `entryDir` and we'll put an entry _MultiColor.jsx_ at the root of _react4xp/entries_, the jsxPath becomes just the clean file name.
...
----
<1> The entry imports some secondary react components for nesting later. These imports have nothing to do with react4xp's link:#jsxPath[jsxPath system], this is regular *ES6 importing* and will be handled by webpack at compile time. Any react component can be imported from anywhere here, including other entries. In this case we're referring to components below a different folder, _shared/_ which is not below _entries/_ but adjacent to it, and will be marked as a *chunk directory* instead of an entry directory: so these imported react components will not be entries, just secondary react components (not reachable with `React4xp.render`). As you will see, this *folder separation* is also important for the compiled outcome: this entry will be compiled into _assets/react4xp/MultiColor.js_, but imported components like these will be collected inside a common *dependency chunk*: _assets/react4xp/shared.<HASH>.js_. React4xp and webpack track the dependency connections between an entry and all its required chunks - so `React4xp.render` automatically generates asset-loading page contributions from the controller. The outcome: when the webpack rules are set up correctly later, *imports like this in entries rendered by react4xp are all that's needed to connect the entries to the additional assets they might need*: no manual url handling in the controller or thymeleaf views etc. Dependency chunks are also  optimized for repeated use.
...
<6> This makes the `MultiColors` object available as `this` inside the functions `shiftUp` and `shiftDown`.
...
<8> If no colors have been added editorially, the part just displays an _Add some color!_ `<p>` (12). Otherwise, the (somewhat) interesting stuff is rendered:
...
<12> The fallback rendering if no colors have been added.
<13> Remember that link:#entries[entries need to export a function] (_props? => react component_). Since this component is written in the class syntax (4), it needs to be exported like this.
...
And finally a *ColorButtons* component. It takes the list of `props.colors` (from the editorial data), and assigns each of them to a `Button` labeled with the color, each of them triggering the change-color-selection from _MultiColor.jsx_ with its own specific color index `i` in the list of colors. In addition, the HTML class name(s) of the buttons is assigned dynamically: the buttons all have a `color-button` class, and each item checks if its own index matches the currently selected index, and if it does, the class `selected` is added - which is differently styled than the unselected ones:
...
First, open _react4xp.properties_ in your project root and look up the attributes `entryDirs` and `chunkDirs`. These values are comma-separated lists, so make sure `entries` are added as an `entryDir` and `shared` is added as a `chunkDir`:
...
chunkDirs = shared    <!--2-->
----
<1> `entryDirs` is a list of where react4xp will look for .JSX (and .ES6/.JS) source files to compile and serve as entries. This is in addition to XP's _site/_ folder, which is a special-case default entryDir. Under _site/_ only .JSX files are picked up as entries.
<2> `chunkDirs` is a list of folders that will be turned into collection/library assets. One chunk asset for each file type in each chunkDir, named after the closest parent chunkDir.

So in our example here, JSX files under _entries/_ are available to `React4xp.render` and friends, in XP controllers. And everything they import from under _shared/_ and that is compiled into JS (both react and otherwise), will be collected into the chunk `shared.<HASH>.js` - and everything that's compiled into CSS (our SASS, for example) is collected into `shared.<HASH>.css`. And so on.

Both `entrydirs` and `chunkDirs` are comma-separated *lists of relative paths*, relative to _src/main/resource/react4xp/_.

What happens to imports into entries from folders not marked as chunkDirs, then?

- Importing an entry into anonther entry is possible, and leaves both entries as they are.
- Importing JS like react etc, _will be collected into the compiled entry asset!_.
- Other imports, such as CSS, are compiled to separate assets under their own name.

{zwsp} +

Next, we'll add a *webpack config file* reference to the same _react4xp.properties_ and add that file:

GJØØØR DEEEET DAAAA. GJØØØØR DEEEET NÅÅÅÅÅÅ.


----
HERE BE DRAGONS!

Everything below this box is work in progress / not finished / dangerous
----

----
    TIP: This is why [entries] are used as a separate word from react components, even though they technically _are_ just react components: Webpack makes entries runtime-available to react4xp (entries is even a Webpack term).
----
...
[TIP]
====
*Chunks are most optimized.*

It's recommended to keep the entries slim and put as much of the heavy and reusable stuff as possible into chunks.

Also, it's best to keep the non-entries in chunks: each JSX entry is compiled to its own separate JS file. If an entry imports a non-entry piece of code that's _not in a chunk either_, it will just be compiled into the entry's JS "bundle", making it more heavy-weight.
====
