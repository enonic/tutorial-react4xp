...
For context before we move on with the example, here's a closer look at the `.render` call from the ES6 controller in link:#_code[the example above]:
...
== Client-side, props, part

Now that you know the basic structure, let'do the same thing in a slightly different way - demonstrating these independent variations:

* Wrap the react rendering in a *part instead of a page*,
* Use the `clientRender` option to bypass the server-side rendering and go straight to classic *client-side react rendering*,
* Renter react into HTML from a *thymeleaf template*,
