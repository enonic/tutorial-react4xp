...
react4xp.config.json            <!--2-->
react4xp.properties             <!--3-->
...
<2> _react4xp.config.json_: Place to add extra externals
<3> _react4xp.properties_: tune the most common properties of the react4xp project.
<4> The _react4xp/_ folder is opt-in: can be ignored for now, and doesn't need to contain anything. But when fine-tuning the build later in the tutorial, we'll get back to why you might find it handy to have a separate place for some of your react components (in ultra-short: the _entries/_ folder is for react components that can be used directly by react4xp, the _shared\_ folder is for common components imported by, and shared between, the entries).
<5> Under _headless/_, the starter includes two wrappers that make it easier to use the optional guillotine api for using react4xp in a <<guillotine#, headless context>>.
