...
And finally a *ColorButtons* component. It takes the list of `props.colors` (from the editorial data), and assigns each of them to a `Button` labeled with the color, each of them triggering the change-color-selection from _MultiColor.jsx_ with its own specific color index `i` in the list of colors. In addition, the HTML class name(s) is assigned dynamically: the buttons all have a `color` class, and each button checks if its own index matches the currently selected index, and if it does, the class `selected` is added - which is differently styled than the unselected ones:
