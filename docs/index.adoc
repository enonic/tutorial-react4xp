...
[[props_clientside]]
== 2: Editorial props, clientside render
...
----
<1> This (line 23) is all that's needed: a truthy `clientRender` flag in the _options_ object makes the server skip the HTML rendering (in _this_ particular call. Mixing up clientside and serverside rendering across different places hasn't been tested very much, but it should work fine). And on the client, `React4xp.CLIENT.render` (which wraps `react.render`) is called instead of `React4xp.CLIENT.hydrate` - making the browser fill the target element DOM from scratch.
...
Although, if you think that _makeObjectDropper_ closure thing is a strange way to do something react itself could do better... I won't argue. Just trying to focus on one thing at a time.
====

Next, we'll modify *the controller* to inject data into the react component:
...
<2> This makes `React4xp.render` pretty analogous to XP's `thymeleaf.render` template engine. The first argument is just a reference to what should be rendered (_react component_ ~ _template_), the second one is a collection of data injected into it (_props_ ~ _model_).
<3> To be clear, `clientRender` has nothing to do with props one way or the other.
...
[.thumb]
image:gday_bruce.png[title="Rendering: G'day Bruce. This is all just one big philosophers' joke, isn't it?", width=768px]
