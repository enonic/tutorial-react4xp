...
In this example we'll make another Part, and re-use the Page controller and content type we made previously *LIIIIINK*.

However, this time we won't use the `React4xp.render` shorthand function. Instead, we'll look at the *explicit react4xp syntax* and construct *a data-holding React4xp object*. It gives you better control and opportunities for logic steps when you handle your entry; you can manipulate it or extract data _before_ rendering. And the rendering is done with *two separate rendering methods* for the `body` and `pageContributions` in the controller response.

We will also use a thymeleaf template - and take advantage of the react4xp data object to send its ID to the thymeleaf model - just to demonstrate a way to combine react and thymeleaf rendering.

And we will use the link:#jsxPath[_jsxPath_ as described above] to point to an entry in a different location - we'll actually just recycle the entry from the previous part *LIIIIINK*.
