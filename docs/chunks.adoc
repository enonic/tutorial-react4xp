...
== Importing
Entries and other chunks can import resources from chunks using *ordinary ES6 imports*, as long as the necessary webpack rules are added. Compilation rules for regular ES6 and JSX are included with react4xp. Other rules (loaders etc) you must add yourself - see link:#imports_chunks[example 6].
...
Dependency chunks never contain link:#entries[entries]; each entry is an asset file of its own. Consequently, nothing inside a dependency chunk will have a jsxPath.
