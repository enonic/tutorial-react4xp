...
With a serverside rendering (`options.clientRender` is falsy), the client will expect an existing target element with a pre-rendered entry in the response HTML, and call `react4xp.CLIENT.hydrate`. If `true` / truthy, an empty target element is expected in the response HTML, and the rendering is left to the client with `react4xp.CLIENT.render`.
