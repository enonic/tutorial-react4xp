...
<2> `chunkDirs` is a list of folders that will be turned into collection/library assets (dependency chunks): in this case _src/main/resource/react4xp/shared_. One chunk asset for each file type in each chunkDir, named after the chunkDir. If a different name is already here: this is a comma-separated file, so for example `entryDris = entries, myEntries` works fine.
...
----
npm install -D css-loader@3 node-sass@4 mini-css-extract-plugin
----
