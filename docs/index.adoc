...
For flexibility, there are several ways to use React4xp. They can be boiled down to *4 basic patterns*. You're not bound to use them exactly like this of course, and the *link:#_examples[examples]* below will show some more variation than this. But for clarity here's an overview:
...
React4xp has two _shorthand functions_ (shortcut syntaxes doing the most common stuff all at once): `.renderSafe` for purely client-side rendering, and `.render` for server-side rendering and client-side hydration. These are built to be easy to use and syntactically similar to how you would normally use Thymeleaf etc in XP. Parameters are tweakable, and they both return *a full response object with everything the client needs to display it* - HTML and page contributions. Just return it directly from the controller.

{nbsp} +

=== B: JsxPath references
If you want a part/page to use an entry component from a different folder, or reuse an entry across multiple parts or pages, then *each entry has a jsxPath used to refer to it from any page/part*:

  - Either put a `.JS`/`.JSX`/`.ES6` file under `react4xp/_entries`,
  - or a `.JSX` file in any page/part folder under `site/`,

The jsxPath will be the file name relative to these - after compilation you can see all the built jsxPaths in `build/resources/main/assets/react4xp/entries.json`. Then, `.render` and `.renderSafe` (and the React4xp object constructor, see below) can receive a jsxPath string instead of the `getComponent`.

NOTE: JsxPaths are not OS paths that can be used relatively. They are generated *name strings, and must be used in full*.

{nbsp} +

=== C: React4xp data objects
If you need *more flexibility*, for example want to use _more than one_ entry component in a single part or page, or even use _the same entry component more than once_ in the same part/page:

Instead of using the shorthands `.render` or `.renderSafe`, create a temporary data-holding `new React4xp(...)` object in the page/part controller, with a component or jsxPath argument to point to the react component. When you're done manipulating data, the React4xp object has built-in and specific methods for rendering a body and the necessary page contributions, for both client-side or server-side react rendering.

These rendering methods can be _chained_, so that the output HTML or page contribution from one object is the input for the next one. This way, one response object can be built from several react components.

{nbsp} +

=== D: Client-side exposed services
Finally, React4xp can be *used outside of the page/part flow*, even without XP controllers if needed. A few services are exposed in runtime, where a client can request runnable compiled code:
...
<4> Downloading the react component itself - the compiled version of `01-minimal-example.jsx`. The runtime-renderable component is put into `React4xp.<jsxPath>`, in this case: `React4xp['site/parts/01-minimal-example/01-minimal-example']`.
<5> A client rendering command `React4xp.CLIENT.render` is called with some arguments: A pointer to the react component, the ID of the element to render the component into, and the props. Note that we only added the `greetee` prop. The other one, `React4xpId`, is the unique ID of the component, same as the element target. It's always added as a react prop for each component, helping to separate multiple instances of the same component (for example allowing them to share a common redux store without meddling with each other's state).
