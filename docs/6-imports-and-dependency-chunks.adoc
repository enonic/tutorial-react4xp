...
<3> This one is a little bit different compared to previously. `"MultiColor"` is a link:#jsxPath[jsxPath] to an entry, but why does it look so... naked? Where does it point to? As we'll see link:#[webpack_config[later], we'll set up react4xp to look for entries below an `entryDir` in react4xp.properties: _src/main/resources/react4xp/myEntries/_. Since a jsxPath is relative to the closest parent `entryDir` and we'll put an entry _MultiColor.jsx_ at the root of _entries/_, the jsxPath becomes just the clean file name.
...
-> link:entries-and-jsxpath.html#chunks[Dependency chunks]
