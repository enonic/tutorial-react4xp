...
XP uses _webapp.ts_ and _webapp.html_ to generate an initial HTML that directly makes the browser run most of <<#nutshell, the steps above>>, fetching assets and setting up initial values, and then calling the final _script.js_ asset, which handles the rest.
...
TIP: If you've been through the lesson in the previous chapter, you might recognize that *the logic in is script is the same* - the functions here are just copied or slightly modified from <<guillotine#movielistrequests, _movieListRequests.ts_>> and <<guillotine#guillotine_request_helper, _guillotineRequest.ts_>> - see those for more reference.
