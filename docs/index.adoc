...
- Line 10: The `.render` shorthand method produces a server-side rendering. In live view, it also hydrates the rendered HTML - post-filling it with the active react and js functionality on the client.
...
- Line 16: the `body` parameter is an HTML string. Here, it's rendered by Thymeleaf based on the view file above. The `body` parameter is *pass-through*: had this been the `.renderSafe` client-side rendering function, it would just have become the `body` in the response output, only with a container element added (possibly. Nothing's added if a matching-`id` element is found in it). But with `.render`, the react component is rendered to HTML by the server using the initial `props`, and inserted into the target container in `body` (which, again, might be a generated target container if the input HTML is missing one). This modified `body` is then returned. On the client, `.render` will `.hydrate` the container, filling in and activating the react component.
