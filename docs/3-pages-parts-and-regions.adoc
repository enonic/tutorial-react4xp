= React4xp lesson 3: XP content studio -  pages, parts, regions and content
:toc: right
:toclevels: 3
:imagesdir: media/
...
Before we move on to the XP Part, we need to bind this Page controller to the _my-content_ Content Type we just made, in the regular XP way: a link:https://developer.enonic.com/docs/xp/stable/cms#page_templates[Page Template].
...
[start=4]
4. Apply/save and you're done. Now every new _myContent_ item in every site with this Template will be rendered with the _default_ page controller.
...
(Just for the record, there is an alternative renderer, `.renderPageContributions`. It also starts a new react app, but never needs `request`. More on that in a <<5-custom-flow-syntax#, later lesson>>)
====
...
2. <<2-editorial-data-and-props#, Editorial data and props>>|

Next: advanced ->

4. <<4-source-file-structure#, Free source file structure>>
|====
