...
3. In addition to chain-rendering more than one entry, we will use the link:#jsxPath[_jsxPath_] as described above to point to entries in a *different location*. Actually, let's just recycle the entries from before:
...
<2> The already existing entry link:#color_jsx[_color.jsx_]: different location, and different name.
<3> We're also reusing the existing entry link:#hello_react_jsx_modified[_hello-react.jsx_] (with props, not the first hardcoded one), located under _site/*pages*/hello-react/_ - even though what we're making here is a Part. Location doesn't matter: when using jsxPaths, all entries are equal.
...
Next, we're adding a *thymeleaf view template*. This will get the `color` data from the same source as the _color.jsx_ entry: from the definition above. The entry will be rendered into the `<div>` container based on the element ID. The `react4xpId` value will come from the react4xp data-holding object itself:
...
Finally, *the controller*, a bit juicier than before:
...
exports.get = function(request) {
    ...
    const finalPageContributions = helloObj.renderPageContributions({
        pageContributions: colorPageContributions                            <!--10-->
    })


    // Determining if the rendering context is not inside Content Studio:
    const isOutsideContentStudio = (                                         <!--11-->
        request.mode === 'live' ||
