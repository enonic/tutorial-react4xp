...
In this chapter you will be introduced to how controllers may use TSX/JSX templates for both server-side and client-side rendering


[[code]]
== Code

We need to update the application with some code to get things going:

. Start by adding this React component (tsx) file:
...
This component will be rendered server-side, and when `hydrated` it will also work on the client-side, for an amusing beer bottle countdown. The actual updating step is done with vanilla JS instead of actual react, just to keep things as simple as possible. We'll look at stateful/dynamic components later.

. Then add an Enonic controller (ts) to your app:
...
<7> `ssr` true means the component will be Server Side Rendered


. Finally, add the following mapping to your site.xml - this will trigger the controller when a site is previewed:
...
[NOTE]
====
A quick gotcha on using `document` and `window` etc during serverside rendering:
