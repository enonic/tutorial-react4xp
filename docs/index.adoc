...
The `props` of course come from the *Page controller*. Still similar to the previous examples, but we're adding props that are needed by `<Region>` in the react component.
...
TIP: Adding an XP Region to a Layout is exactly the same - only easier: since it will be injected inside a Page, you don't need to add an outer `body` in the _options_ parameter, and therefore you don't need an `id` parameter either - same as in the XP Part controller below.

==== XP Part

Now let's add a simple Part to drop into the Region: _site/parts/color/_ .

*The Part definition* is still pretty unremarkable, there's a `color` TextLine input field in the form:
...
Here we don't even need the _options_ argument (the object with `clientRender` and `pageContributions`), it can be completely skipped. But just for demonstrating, let's see that this part is _independent_ - both from the _default_ Page we defined a moment ago, and from any other Parts dropped into the same Region:

<1> The part is clientside rendered...
<2> ...and adds a tiny script with the same color to the page: by passing `pageContributions` through the _options_ parameter, the same way we've passed `body` through it so far.
