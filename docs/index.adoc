...
*The controller* is now juicier than before. Summary of what to look for:

- Each entry is used to set up a data-holding reactxp object, in steps _before the rendering_ is called.
- In these steps, data (options and props) are both injected into them, and extracted from them. The `targetId` value in the thymeleaf template above, for example, is generated by react4xp and extracted from the react4xp object.
...
<2> `setProps` modifies `helloObj`, to add some `props`.
<3> After creating a second react4xp object, `colorObj`,
