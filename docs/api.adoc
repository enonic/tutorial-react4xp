...
With a serverside rendering (`options.clientRender` is falsy), the client will expect an existing target element with a pre-rendered entry in the response `body`, and call `hydrate`. If `options.clientRender` is truthy, an empty target element is expected in the response `body`, and the rendering is left to the client with `render`.
...
The `react4xp.properties` file has been removed and it's settings moved to other files, see the <<upgrade#,upgrade guide>>.
...
- Of course, this requires that the link:https://www.npmjs.com/package/@enonic/react4xp[react4xp NPM package] is installed at the time gradle (or the enonic CLI) is run.
+
If you want to automate that, you can always add this to your _gradle.build_:
+
[source,groovy,options="nowrap"]
----
def react4xpGradleFile = new File('node_modules/@enonic/react4xp/react4xp.gradle')
...
- Or, reverse-engineer the whole thing from link:https://github.com/enonic/enonic-react4xp/blob/master/react4xp.gradle[react4xp.gradle] and make your own gradle adaptation.
...
react4xp.ssr.settings = --persistent-code-cache, --class-cache-size=42, --lazy-compilation
...
By defauilt Graal.js engine is selected, but only if it is available. Otherwise first available JavaScript engine is selected (for XP on OpenJDK 11 it is usually Nashorn. 
