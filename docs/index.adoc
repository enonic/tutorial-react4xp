...
Being a familiar with https://webpack.js.org/guides/getting-started/[webpack] isn't strictly necessary, but might make the structure make sense.
====
...
<5> Two `REPLACE_ME.jsx` files under `react4xp/_entries/` and `site/`. Webpack needs these empty placeholders here, you can remove each of them when you've added one or more React source files under those folders.
...
If you need it later, https://github.com/enonic/lib-react4xp/blob/master/README.md#installing-the-library[the React4XP documentation is right here].
...
TIP: The example code tries to speak for itself. If something isn't clear, there's plenty of explanations in between. If you're really good at this, you will probably skip through a lot of the explanations. That's fine, but you might want to take a look at *the most central terms*: link:#_entries[entries], link:#_jsxpath[jsxPaths], and link:#_chunks[chunks].

TIP: You can copy/paste the code examples into the starter, or download the examples along with everything needed to run, by using git and checking out the https://github.com/enonic/starter-react4xp/tree/examples[examples branch of starter-react4xp].


{nbsp} +
{nbsp} +

=== 1: Hello World (entries, server-side rendering by default)
...
Now for *the React component* itself - _the entry_:
...
- is placed either under the folder `/react4xp/_entries`, or like in this example: in a part or page folder under `site/`.

The returned function can take an optional `props` object parameter. Our entry will receive a `greetee` prop and greet it with a booming _Hello_.
...
[.thumb]
image::hello-world.png["A most glorious greeting of the very concept of existence itself! In suitably divine Times New Roman. Because a good tutorial should never have ANY irrelevant added layers!!!" width=492px]
...
=== 2: Parameters (jsxPath, client-side rendering, target body, editorial props)

In this example we'll create another part, similar to link:#_1_hello_world_entries_server_side_rendering_by_default[the first example], but with these variations:
...
TIP: We've hardcoded the ID here and in the controller for clarity. The best practice however, would be to pass it to thymeleaf through the model - see link:#_3_react4xp_objects_alternative_syntax_manual_flow_control_the_entries_folder[example #3].
...
exports.get = function(request) {
    ...
    const jsxPath = 'site/parts/01-minimal-example/01-minimal-example'; <!--4-->
...
=== 3: React4xp objects (alternative syntax, manual flow control, the _entries folder)
...
react4xp/_entries/
  ColorThing.jsx
site/parts//03-customflow-example/
...
=== 4: Multiple entries (chaining, nesting, chunks)
...
.site/parts/04-chaining-example/04-chaining-example.jsx:
[source,javascript,linenums,options="nowrap"]
----
import React from 'react';
...
.site/parts/04-chaining-example/04-chaining-example.xml:
[source,xml,linenums,options="nowrap"]
----
<part>
...
.site/parts/04-chaining-example/04-chaining-example.html:
[source,html,linenums,options="nowrap"]
----
...
.site/parts/04-chaining-example/04-chaining-example.es6:
[source,javascript,linenums,options="nowrap"]
----
const portal = require('/lib/xp/portal');
const thymeleaf = require('/lib/thymeleaf');
...
exports.get = function(request) {
    ...
    ['first', 'second', 'third', 'fourth'].forEach(cardinalNum => {	<!--7-->
        const notUniqueComp = new React4xp(
            	'site/parts/01-minimal-example/01-minimal-example'
            )
...
In other words: we can use an _entry_ multiple times targeting different container elements by using different React4XP objects. And we can render the same _React4XP object_ into the same container multiple times (if we should need to). But *trying to change the ID and target element will throw an error after an object's first rendering.*
====
...
<html>
    ...
    <body>
        ...
        <script src=" (...) /react4xp/site/parts/04-chaining-example/04-chaining-example.js"></script>  <!--5-->
        <script defer>
            React4xp.CLIENT.render(
                React4xp['site/parts/04-chaining-example/04-chaining-example'],
        ...
        <script src=" (...) /react4xp/site/parts/01-minimal-example/01-minimal-example.js"></script> <!--5-->
        <script defer>
            React4xp.CLIENT.hydrate(
                React4xp['site/parts/01-minimal-example/01-minimal-example'], <!--3-->
                "this-is-not-unique",
                {
...
Thats _most_ of what React4XP offers. For more technical details, some corner-case features, adaptability and more complex functionality, you can take a look at link:#[the API overview] or the link:#[React4XP library documentation].
