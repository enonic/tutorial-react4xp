...
Before we move on to the XP Part, let's connect this Page controller to the _my-content_ Content Type we just made, for this example and the later ones. If you're familiar with XP and Content Studio, this is nothing special: the regular way of making a Page Template that binds our Content Type to our new Page controller.

When this is done, *using the _Default_ Page controller will be easy*: create a new _my-content_ item in your site, and it will come out ready to use with _Default_.
...
Now we're ready to add a simple Part to drop into the Region: _site/parts/color/_ .
...
Like before, *the controller* mainly hooks the editorial data into the props:
...
Actually, we don't really need the _options_ argument at all here (the object with `clientRender` and `pageContributions`), it can be completely skipped. But for demonstrating, let's use it to see that this part is _independent_ - both from the _default_ Page we defined a moment ago, and from any other Parts dropped into the same Region, react or otherwise:

<1> The part is clientside rendered...
<2> ...and adds a tiny script with the color: using the _options_ parameter to pass `pageContributions` through `.render`, the same way we've passed `body` through it so far.

TIP: You can skip the `body` _option_ like we did here, to just have react4xp generate an HTML container element. `body` is only needed if you need the controller to _add_ HTML around the rendered react. The ID will match the auto-generated container element, you can set the ID value with the `id` option - or skip that too and just let react4xp supply a unique ID. See the link:#_react4xp_render[React4xp.render] box.
...
1. In the same Site as the Template you just made (but not under _Templates_), create a new _my-content_ item:
...
NOTE: As we'll come back to in more detail link:#entries_and_jsxpath[later], each time `React.render` is called (with the _request_ argument) it will create a separate _root react app_. In this case, it will be one react app for each of the _color_ Parts you added to the Region.

[start=5]
5. As usual, clicking _Preview_ on the top will open a new Preview tab for your page.

The *HTML output* of all this? It's structurally and conceptually the same as in the previous examples. Looking at the _Page Source_, it should all make sense to you by now.
