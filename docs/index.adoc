...
NOTE: The examples assume you already have a little basic experience Enonic XP 7 (if not, check out the excellent introductions at the https://developer.enonic.com/[Developer Portal]), and they _build incrementally on each other from there_. You should also have a https://developer.enonic.com/guides/my-first-site#page_component[main page component] with a https://developer.enonic.com/guides/my-first-site#regions[region] in the same project.

TIP: See the link:#_api[API section] for more details about how to use what's demonstrated.

TIP: These examples can be downloaded by checking out the https://github.com/enonic/starter-react4xp/tree/examples[examples branch of starter-react4xp], along with everything needed to run.
...
Run `enonic project deploy` from your shell, start the sandbox and direct your browser to `localhost:8080/admin`. Log in to XP and open Content Studio. If you haven't already, create some content (a site or the included landing page type will do) that uses a page controller with a region. Insert a new part, and choose the minimal-example part you just made. Edit it, give it a displayname, save it and enter the preview:
