...
TIP: This structure is just an overview, and you don't need to know it by heart to get started. For now, *just put your JSX source files under `react4xp/entries/` or under `site/`, and you're ready to build and run.*
...
<2> `react4xp.properties`: this is where you can tune the most common properties of the react4xp project.
<3> The `react4xp/` folder is opt-in: it doesn't need to contain anything, but when fine-tuning the build later we'll get back to why you might find it handy to have a separate place for some of your react components. The super-short version is that the `entries\` folder can have react components that can be used by react4xp, and the have `shared\` folder can contain common components shared between entries (entries can import any other react components).
<4> A bare-bone page controller is included with the starter - both for convenience in general, and as a sandbox for the first simple examples here,
<5> Also a simple part with a pre-made react view, for some other examples.
