...
These files are not included in the starter - just use your copy/paste skills and remember that `hello-react` must be identical between file names and folder name.
...
[.thumb]
image::hello-world.png["Rendering: Hello World. In fabulous Times New Roman, because no good tutorial should have irrelevant complexity, and this is a good tutorial.", width=492px]
...
<1> At the top, we see the `<div id="react4xpApp">` target container, now filled with a server-side rendering of the react component.
...
<3> The second loaded asset is a client-wrapper, available in the browser's namespace as `React4xp.CLIENT`. You can read the link:https://www.npmjs.com/package/react4xp-runtime-client[client wrapper docs on NPM].
<4> The third asset is the compiled version of _hello-react.jsx_. React4xp used the `component` data to resolve the link:#_jsxpath[jsxPath] that locates the asset after compiling: `site/pages/hello-react/hello-react`. The jsxPath is also used when loading the asset into the the browser's namespace as `React4xp['site/pages/hello-react/hello-react']`.
<5> Finally the trigger. The CLIENT wrapper calls link:https://www.npmjs.com/package/react4xp-runtime-client#hydrate[hydrate] to activate react in the serverside-rendered asset `React4xp['site/pages/hello-react/hello-react']` in the target container `"react4xpApp"`. We haven't added any props yet, hence the `null` argument. The final two digits are internal react4xp housekeeping, flagging some possible need for client-side postprocessing.

== React4xp.render

For context before we move on with the example, here's a closer look at the `.render` call from link:#_code[the example above]:
...
- `entry` (mandatory, string or component object): this is the reference to the "template", the react component to be rendered. If it's a JSX file with the same name in the same folder as an XP component, simply use the `portal.getComponent()` object here. In this example,  it's the _hello-react_ page, but you can do the same thing from part or layout controllers too. We'll come back to a more specific way to reference files anywhere in your project, under link:#_jsxpath[jsx path]. By the way, a react component that is used by react4xp this way is called link:#_entries[an entry].
