...
<4> A bare-bone `default` page controller is included with the starter - both for convenience in general, and as a sandbox for the first simple examples here,
...
One thing is mandatory for this to work, shown in the last line: the JSX file *must default-export a function* that *takes an (optional) props object* and *returns a react object*. Apart from that, you can use ordinary ES6 and import other JS and third-party stuff from node_modules in the regular way (there are a couple of caveats to this, we'll get back to that below).
...
The essential thing here is that at the end of the `get` function, *react4xp in a single _.render_ call creates a _full_ XP response object, that makes react work.*

A few more details on what's going on here:

<1> In line 2, `React4xp` is imported from the library.
<2> In line 4, we pick up the `request` data. This is important later.
<3> In line 5, we get the XP component data. This is a simple way to point react4xp to "this XP component". In this case, the _hello-react_ page, but you can do the same thing from a part or a layout controller. The result? `React4xp.render` uses this to search in the same folder (site/pages/hello-react/) for react components with the XP component's own name: _hello-react.jsx_.
  A react component that is used by react4xp this way is called *an entry*. We'll come back to that, and to a different way of referring to specific entries, anywhere in your code.
<4> In line 8, the juicy bit: *the actual render call*. The rendered response from it can be returned directly from the `get` function, since `.render` creates both an HTML string `body`, and a `pageContributions` attribute with everything needed: react (and react-dom), the asset _hello-react.js_ (automatically compiled from _hello-react.jsx_, and served and optimized by react4xp), and the react code to activate the component in the client. `React4xp.render` has the following *signature*: `React4xp.render(entry, props, request, options);` - we'll get back to that later, as well as other, more detailed ways of rendering.
<5> In line 13, an `id` attribute is set in the options object (lines 12-20). This is the id we're giving to the react component, and the HTML id of the target element where react will render the component in the end. Note that the same ID is found in an element in the HTML `body`:
<6> In lines 14-19, we're adding an HTML string `body` option, as a base for the output. React4xp _can_ render its own HTML without this. However, react itself doesn't like to render the tags `<html>`, `<head>`, `<body>` or anything outside of a containing root tag (such as `<!DOCTYPE html>`). So it's easiest here to just hardcode the HTML context here. But this string could just as well have come from a Thymeleaf rendering, for example. Just make sure the `id` option matches an HTML id in `body` (what happens if not? React4xp creates a target element with that ID last in the root element of `body`. Or if there's no `body`, a matching-ID `<div>` is created. Or if there's not even an `id`, a random ID is created).
