...
<4> See how this makes `React4xp.render` pretty analogous to XP's `thymeleaf.render` template engine? The first argument, `entry`, is just a reference to what should be rendered (_react component_ ~ _template_), and the second one is a collection of data injected into it (_props_ ~ _model_).
...
[.thumb]
image:hello_cs.png[title="Editing Hello World, same as before. You actually CAN click this image, to see a high-res version", width=1024px]
...
So now, it looks the same as before, but with editorial data instead of hardcoded text. This is too similar to the previous example; just repeating "Hello World" might cause a little confusion. *Try adding your own data in the fields*, for example changing _"world"_ into _"link:https://montypython.fandom.com/wiki/Bruces_sketch[Bruce]"_ etc, to keep it clear.
...
----
<1> The target container `react4xpApp` now comes filled from the server, with a rendering with your texts from `props` already inserted.
<2> There's still no change in the asset URLs, but since we changed _hello-react.jsx_, the _content_ of _hello-react.js_ has of course been recompiled.
